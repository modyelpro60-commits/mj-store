  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* ══ PERIMETER CHARGE ══════════════════════════════════════ */}
        <motion.span aria-hidden className="pointer-events-none absolute top-0 left-0 h-px w-full bg-purple-400" style={{ transformOrigin: "left" }}
          initial={{ scaleX: 0, opacity: 0.28 }} animate={{ scaleX: hovered ? 1 : 0, opacity: hovered ? 1 : 0.28 }} transition={{ duration: 0.28 }} />
        <motion.span aria-hidden className="pointer-events-none absolute right-0 top-[24px] w-px bg-purple-400" style={{ height: "calc(100% - 24px)", transformOrigin: "top" }}
          initial={{ scaleY: 0, opacity: 0.28 }} animate={{ scaleY: hovered ? 1 : 0, opacity: hovered ? 1 : 0.28 }} transition={{ duration: 0.28, delay: hovered ? 0.22 : 0 }} />
        <motion.span aria-hidden className="pointer-events-none absolute bottom-0 right-0 h-px w-full bg-fuchsia-400" style={{ transformOrigin: "right" }}
          initial={{ scaleX: 0, opacity: 0.28 }} animate={{ scaleX: hovered ? 1 : 0, opacity: hovered ? 1 : 0.28 }} transition={{ duration: 0.28, delay: hovered ? 0.44 : 0 }} />
        <motion.span aria-hidden className="pointer-events-none absolute left-0 bottom-0 w-px h-full bg-fuchsia-400" style={{ transformOrigin: "bottom" }}
          initial={{ scaleY: 0, opacity: 0.28 }} animate={{ scaleY: hovered ? 1 : 0, opacity: hovered ? 1 : 0.28 }} transition={{ duration: 0.28, delay: hovered ? 0.66 : 0 }} />
        ...
        <motion.div aria-hidden
          ...
          initial={{ opacity: 0.25 }} animate={{ opacity: hovered ? 0.9 : 0.25 }} transition={{ duration: 0.35 }}
        />
        ...
        <Link href={`/product/${product.id}`} className="relative flex flex-col flex-1 p-5 gap-4">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {/* Discount badge — shown next to tags if discount exists */}
            {discountPct > 0 && (
              <span className="mono-label text-red-300 border border-red-500/35 bg-red-500/[0.12] px-1.5 py-[3px]"
                style={{ borderRadius: "var(--radius-badge)" }}>
                -{discountPct}% OFF
              </span>
            )}
          ...
          <div
            ...
            style={{
              ...
              background: "radial-gradient(circle at 50% 38%, rgba(124,58,237,0.16) 0%, rgba(10,10,28,0) 68%), linear-gradient(180deg, #0B0B1E 0%, #070714 100%)",
              boxShadow: "inset 0 0 0 1px rgba(255,255,255,0.04), inset 0 -22px 44px rgba(0,0,0,0.32)",
            }}
          >
            {product.image ? (
            ...
            {/* Bottom depth fade */}
            <span aria-hidden className="pointer-events-none absolute inset-x-0 bottom-0 h-12 bg-gradient-to-t from-black/45 to-transparent" />
          ...
          {/* ── Name ── */}
          <h3 className={`font-black tracking-tight text-text-primary leading-tight line-clamp-2 transition-colors duration-200 group-hover:text-white ${size === "dominant" ? "text-xl" : "text-lg"}`}>
            {product.name}
          </h3>

          {/* ── Short description (tagline) ── */}
          {product.short_description ? (
            <p className="text-xs text-purple-300/70 font-semibold -mt-2 line-clamp-1">
              {product.short_description}
            </p>
          ) : (
        ...
        <div className="px-5 pb-5 pt-1 space-y-2">
          {/* Buy Now */}
          <button type="button" onClick={handleBuyNow} disabled={adding || buying}
          ...
          {/* Add to Cart */}
          <button type="button" onClick={handleAdd} disabled={adding || buying}
