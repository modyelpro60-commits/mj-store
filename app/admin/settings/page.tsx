        ...
        >
          {uploading ? <LoaderCircle className="h-4 w-4 animate-spin" /> : (
            <>
              ...
              <span className="text-[9px] font-bold">رفع QR</span>
            </>
          )}
        </div>
      )}
...
function RelativeTime({ iso }: { iso: string | null }) {
  if (!iso) return <span className="text-zinc-600">لم يُستخدم بعد</span>;
  const diff = Date.now() - new Date(iso).getTime();
  const m = Math.floor(diff / 60000);
  const h = Math.floor(diff / 3600000);
  const d = Math.floor(diff / 86400000);
  const label = m < 1 ? "الآن" : m < 60 ? `${m}د` : h < 24 ? `${h}س` : `${d}ي`;
...
function SettingsPageInner() {
  ...
  async function uploadQR(file: File, key: string, onDone: (url: string) => void) {
    ...
    try {
      ...
      if (data.success && data.url) { onDone(data.url); toast.success("تم رفع الصورة"); }
  ...
  async function toggleActive(acc: PaymentAccount) {
    ...
    try {
      ...
      if (data.success && data.account) {
        ...
        toast.success(data.account.is_active ? "تم تفعيل الحساب" : "تم تعطيل الحساب");
      } else {
        toast.error(data.error ?? "فشل تغيير الحالة");
      }
    } catch {
  ...
  async function saveEdit(id: string) {
    if (!editVal.trim()) { toast.error("القيمة مطلوبة"); return; }
    setSavingId(id);
    try {
  ...
  async function submitAdd(method: string) {
    if (!addVal.trim()) { toast.error("القيمة مطلوبة"); return; }
    setAddSaving(true);
    try {
      const res  = await fetch("/api/admin/payment-accounts", {
        method:  "POST",
    ...
    <div className="space-y-8 pb-12" dir="rtl">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-black text-white">طرق الدفع</h1>
            <p className="text-xs text-zinc-600">Vodafone Cash · InstaPay · USDT · إعدادات مالية</p>
          </div>
        </div>
        <button
          type="button"
        ...
        >
          ...
          {savingGlobal ? "جاري الحفظ…" : "حفظ الإعدادات العامة"}
        </button>
      </div>
      ...
      <section className="space-y-3">
        ...
        <div className="grid gap-3 sm:grid-cols-3">
          {([
            { id: "vodafone", label: "Vodafone Cash", badge: "دفع محلي",        icon: Smartphone, enabled: vodafoneEnabled, setEnabled: setVodafoneEnabled },
            { id: "instapay", label: "InstaPay",      badge: "دفع محلي",        icon: Camera,     enabled: instapayEnabled, setEnabled: setInstapayEnabled },
      ...
      {(["vodafone", "instapay", "usdt"] as const).map((method) => {
        ...
        const valuePlaceholder = isVf ? "01XXXXXXXXX" : isIp ? "username (بدون @)" : "0xXXXXXXXX…";
        const valueLabel = isVf ? "رقم الهاتف" : isIp ? "اسم المستخدم" : "عنوان المحفظة";
              ...
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-zinc-600">
                    {methodAccounts.length} حساب ·{" "}
                    {methodAccounts.filter((a) => a.is_active).length} مفعّل
                  </p>
                </div>
              </div>
              {addingMethod !== method && (
            ...
            <div className="space-y-2">
              {loadingAccounts && methodAccounts.length === 0 ? (
                ...
                <div className="flex flex-col items-center gap-2 rounded-2xl border border-dashed border-white/[0.07] bg-zinc-900/20 py-8 text-center text-zinc-600">
                  ...
                  <p className="text-xs font-semibold">لا توجد حسابات بعد</p>
                  <p className="text-[10px]">اضغط "إضافة حساب" لإضافة أول حساب</p>
                </div>
              ) : (
                  ...
                  >
                    {editId === acc.id ? (
                      ...
                      <div className="space-y-4 p-4">
                        <p className="text-[10px] font-black uppercase tracking-wider text-purple-400">تعديل الحساب</p>
                        <div className={`grid gap-3 ${showQR ? "sm:grid-cols-[1fr_1fr_auto]" : "sm:grid-cols-2"}`}>
                          <div>
                            <label className="mb-1 block text-[11px] font-bold text-zinc-500">الاسم (اختياري)</label>
                            <input value={editName} onChange={(e) => setEditName(e.target.value)}
                              placeholder="مثال: الحساب الرئيسي" className={inp} />
                          </div>
                          <div>
                      ...
                      <div className="flex items-center gap-3 px-4 py-3">
                        <p className="flex-1 text-sm font-bold text-red-300">تأكيد حذف "{acc.value}" ؟</p>
                        <button
                          type="button"
              ...
              {addingMethod === method && (
                <div className="rounded-2xl border border-emerald-500/20 bg-emerald-500/[0.04] p-4 space-y-4">
                  <p className="text-[10px] font-black uppercase tracking-wider text-emerald-400">إضافة حساب جديد</p>
                  <div className={`grid gap-3 ${showQR ? "sm:grid-cols-[1fr_1fr_auto]" : "sm:grid-cols-2"}`}>
                    <div>
                      <label className="mb-1 block text-[11px] font-bold text-zinc-500">الاسم (اختياري)</label>
      ...
      <section className="rounded-3xl border border-white/[0.07] bg-zinc-900/50 p-5 space-y-5">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-sm font-black text-white">إعدادات سعر صرف USDT</p>
            <p className="text-[10px] text-zinc-600">يُستخدم لحساب قيمة USDT تلقائياً عند الدفع</p>
          </div>
        </div>

        <div className="grid gap-4 sm:grid-cols-2">
          <div>
            <label className="mb-1.5 block text-[11px] font-bold text-zinc-500">سعر الصرف — كم EGP = 1 USDT</label>
            <div className="relative">
          ...
          <div>
            <label className="mb-1.5 block text-[11px] font-bold text-zinc-500">رسوم التحويل (%)</label>
            <div className="relative">
        ...
        <div className="rounded-2xl border border-purple-500/15 bg-purple-500/[0.05] p-4">
          <p className="text-[10px] font-black uppercase tracking-[0.18em] text-zinc-600 mb-3">معاينة الحساب</p>
          <div className="grid grid-cols-3 gap-3 text-center">
            <div className="rounded-xl border border-white/[0.05] bg-zinc-900/50 px-3 py-2.5">
              <p className="text-[9px] text-zinc-600 uppercase tracking-wider mb-0.5">سعر المنتج (مثال)</p>
              <p className="text-sm font-black text-white">{exampleEgp} EGP</p>
            </div>
            <div className="rounded-xl border border-white/[0.05] bg-zinc-900/50 px-3 py-2.5">
              <p className="text-[9px] text-zinc-600 uppercase tracking-wider mb-0.5">القيمة الأساسية</p>
            ...
            <div className="rounded-xl border border-emerald-500/15 bg-emerald-500/[0.06] px-3 py-2.5">
              <p className="text-[9px] text-zinc-600 uppercase tracking-wider mb-0.5">بعد الرسوم</p>
      ...
      <div className="flex items-center gap-2.5 rounded-2xl border border-amber-500/15 bg-amber-500/[0.04] px-4 py-3">
        ...
        <p className="text-[11px] font-bold text-amber-300/80">
          جميع معاملات USDT على شبكة <span className="text-amber-300">BNB Smart Chain (BEP20)</span> — يتم إعلام العميل تلقائياً في صفحة الدفع.
        </p>
      </div>
